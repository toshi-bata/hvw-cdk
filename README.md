...
> パスを出すため、NGINX 側で**パスを書き換えず（プレフィックスを付けず）そのまま
> アプリへ通過**させます。これにより `/static/…`・`/similarity/…`・`/app/` などが
> すべて解決します。なお**トップ画面の着地 URL はアプリ自身が定めるルート**で、
> HOOPS AI では `http://<EIP>/app/` です（アプリに `/` ルートは無いため素の `/` は
> 404/403 になります。これは正常）。この「無変換の通過」を成立させるため、
> **HVW 静的サイトは同居させない**（`sdkUrl` / `webappPackage` を渡さずにデプロイ）
> のが前提です。また NGINX は元の `Host` ヘッダをアプリへ転送する必要があります
> （アプリは `Host` を使って `thumbnail_url` 等の絶対 URL を生成するため。`@pyapp`
> で `Host`/`X-Forwarded-*` を明示転送済み）。どうしてもサブパスで同居させたい場合
> は、アプリ側が base path 付き URL（FastAPI `root_path` ＋ バンドラの `base` など）
> を出せる必要があり、それはアプリ側の対応事項です。
...
> **アクセス**：デプロイ後、`http://<EIP>/app/`（certbot 後は
> `https://<ドメイン>/app/`）でアプリのトップ画面が開きます（着地 URL はアプリが
> 定めるルート。HOOPS AI では `/app/`）。`/static/…` や `/similarity/…` などアプリ
> の出す絶対 URL はそのまま解決されます。
